...
public protocol ActivityIndicatorViewable: AnyObject {
    ...
    ///
    /// - Note: If the value of this property is true (the default), the receiver sets its isHidden property (UIView) to true when receiver
    ///         is not animating. If the hidesWhenStopped property is false, the receiver is not hidden when animation stops.
    ///         You stop an animating activity indicator with the stopAnimating() method.
